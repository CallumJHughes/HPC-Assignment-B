program main
	...
	real(kind=dp) :: neighbourUp, neighbourDown, neighbourRight, neighbourLeft, phi, start, finish
	...
	time_loop: do t = 1, 100000
		...
		! -14
		if ((avgAfter-avgBefore) .LT. 1.0E-4) completed = .TRUE.
		...
		!if (systemSumAfter-systemSumBefore .LT. 1.0E-14) exit time_loop

		call MPI_Barrier(MPI_COMM_WORLD,ierror)
		if (ierror/=0) stop "ERROR in barrier 1"
